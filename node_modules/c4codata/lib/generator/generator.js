...
function generateSeprateIndexFile(classes) {
    return lodash_1.join(lodash_1.map(lodash_1.uniqBy(classes, function (c) { return c.name; }), // remove duplicated class name, for example CodeList
    function (// remove duplicated class name, for example CodeList
    c) { return "export * from \"./" + c.name + "\""; }), "\n");
}
exports.generateSeprateIndexFile = generateSeprateIndexFile;
function generateSeprateODataFile(uri, user, pass) {
    if (user === void 0) { user = ""; }
    if (pass === void 0) { pass = ""; }
...
exports.generateSeprateODataFile = generateSeprateODataFile;
function generateSeprateClassString(clazz, functionsString, typeString) {
    if (functionsString === void 0) { functionsString = ""; }
    ...
    return lodash_1.join([
        ...
        lodash_1.join(lodash_1.map(lodash_1.filter(clazz.field, function (f) { return lodash_1.startsWith(f.type, "DeferredNavigationProperty"); }), function (ifield) {
            var ToRole = ifield.originProperty.$.ToRole;
...
exports.generateSeprateClassString = generateSeprateClassString;
...
function generateFunctionsString(funcs) {
    return lodash_1.join(lodash_1.map(funcs, function (f) { return generateFunctionString(f); }), "\n");
}
exports.generateFunctionsString = generateFunctionsString;
